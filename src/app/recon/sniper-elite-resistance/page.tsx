...
export default async function SniperEliteResistanceReconPage() {
  const game = await getReconGameBySlug("sniper-elite-resistance");
  if (!game) {
    notFound();
  }
    ...
    <>
      <Section className="pb-8 pt-16">
        <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          <SectionHeading
            title="Sniper Elite: Resistance Recon"
            description="Curated interactive guide layers for Sniper Elite: Resistance. Mission maps stay hidden until their custom schematic maps and marker data are ready."
          />
      ...
      <Section className="pt-4">
        {maps.length === 0 ? (
          <EmptyState
            title="No Sniper Elite: Resistance maps are public yet"
          ...
          <div className="grid gap-4 md:grid-cols-2">
            {maps.map((map) => (
              <Link
                key={map.id}
                href={`/recon/sniper-elite-resistance/${map.slug}`}
